...
class HerdingGame {
    constructor() {
        ...
        this.level = this.getHerdingLevel();
        this.dots = [];
        this.gates = [];
        this.pen = null;
        this.mouseX = 0;
    ...
    awardRewards() {
        // Use EconomyManager to award rewards
        if (window.economyManager) {
            window.economyManager.awardRewards(0, 2, 50, 'herding_game');
        } else {
    ...
    getHerdingLevel() {
        if (window.economyManager && window.economyManager.storageBackend) {
            const level = window.economyManager.storageBackend.getItem('herdingLevel');
    ...
    trackCompletion() {
        if (window.economyManager && window.economyManager.storageBackend) {
            try {
                const completed = JSON.parse(window.economyManager.storageBackend.getItem('herdingCompleted') || '[]');
                completed.push(new Date().toDateString());
                window.economyManager.storageBackend.setItem('herdingCompleted', JSON.stringify(completed));
            } catch (e) {
                console.warn('Failed to track herding completion:', e);
            }
        }
    }
