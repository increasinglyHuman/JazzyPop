...
class GenericCard {
    constructor(config) {
        ...
        this.economics = config.economics || null; // cost and reward info
    ...
    buildCard() {
        ...
        // Economics display (cost/rewards) - always at top if present
        if (this.economics) {
            sections.push(this.buildEconomicsDisplay());
        }
    ...
    buildEconomicsDisplay() {
        const { cost, rewards } = this.economics;
        const canAfford = this.checkAffordability();
        
        return `
            <div class="card-economics ${!canAfford ? 'insufficient' : ''}">
                ${cost ? this.buildCostDisplay(cost, canAfford) : ''}
                ${rewards ? this.buildRewardsDisplay(rewards) : ''}
            </div>
        `;
    }
    
    buildCostDisplay(cost, canAfford) {
        const costItems = [];
        
        if (cost.energy > 0) {
            costItems.push(`
                <span class="cost-item ${!canAfford ? 'insufficient' : ''}">
                    <img src="./src/images/power-icons/energy.svg" class="icon-small" alt="Energy">
        ...
        if (cost.minHearts > 0) {
            costItems.push(`
                <span class="cost-item hearts-required">
                    <img src="./src/images/power-icons/hearts.svg" class="icon-small" alt="Hearts">
                    <span>&gt;${cost.minHearts - 1}</span>
                </span>
            `);
        }
    ...
    buildRewardsDisplay(rewards) {
        const rewardItems = [];
        
        if (rewards.range) {
            if (rewards.range.energy) {
                rewardItems.push(`
                    <span class="reward-item">
                        <span class="reward-range">${rewards.range.energy}</span>
                        <img src="./src/images/power-icons/energy.svg" class="icon-small" alt="Energy">
                    </span>
                `);
            }
            
            if (rewards.range.coins) {
                rewardItems.push(`
                    <span class="reward-item">
                        <span class="reward-range">${rewards.range.coins}</span>
                        <img src="./src/images/coins.svg" class="icon-small" alt="Coins">
                    </span>
                `);
            }
            ...
            if (rewards.range.xp) {
                rewardItems.push(`
                    <span class="reward-item">
                        <span class="reward-range">${rewards.range.xp}</span>
                        <span class="xp-label">XP</span>
                    </span>
                `);
            }
            
            if (rewards.range.gems) {
                rewardItems.push(`
                    <span class="reward-item bonus">
                        <span class="reward-range">${rewards.range.gems}</span>
                        <img src="./src/images/power-icons/gem-base.svg" class="icon-small gem gem-sapphire" alt="Gems">
                    </span>
                `);
            }
        }
    ...
    checkAffordability() {
        if (!this.economics?.cost || !window.economyManager) return true;
        
        const { energy, minHearts } = this.economics.cost;
        const currentEnergy = window.economyManager.displayCache.energy || 0;
        const currentHearts = window.economyManager.displayCache.hearts || 0;
        
        return currentEnergy >= (energy || 0) && currentHearts >= (minHearts || 0);
    }
