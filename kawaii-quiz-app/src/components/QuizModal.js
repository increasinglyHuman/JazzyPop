...
class QuizModal {
    ...
    async open(quizId, config = {}) {
        ...
        if (config.mode) {
            ...
            console.log('Quiz modal opening with mode:', this.mode);
        }
    ...
    async loadQuiz(quizId) {
        try {
            ...
            console.log('Applying mode effects for:', this.mode);
            this.applyModeEffects();
    ...
    showFeedback(isCorrect) {
        ...
        // Handle wrong answer - deduct a heart
        if (!isCorrect) {
            this.deductHeart();
        }
        ...
        if (isCorrect) {
            ...
            if (this.mode === 'chaos') {
                xpEarned = 15; // Bonus for chaos mode
            } else if (this.mode === 'speed' && this.timeRemaining > 20) {
                xpEarned = 20; // Bonus for fast answers in speed mode
            }
            
            // Update dashboard XP display
            this.updateDashboardXP(xpEarned);
            
            // Show XP reward animation
            this.showXPReward(xpEarned);
    ...
    updateDashboardXP(xpEarned) {
        ...
        if (xpElement && levelElement) {
            ...
            // Check for level up (every 100 XP for level 1, 150 for level 2, etc)
            const xpNeeded = 50 + (currentLevel * 50);
            
            if (newXP >= xpNeeded) {
                // Level up!
                newLevel = currentLevel + 1;
                newXP = newXP - xpNeeded; // Reset XP with overflow
            ...
            // Animate the XP update
            this.animateValue(xpElement, currentXP, newXP, 500);
            
            // Save to localStorage
            localStorage.setItem('userXP', newXP);
            
            // Update via API (fire and forget)
            this.updateXPInDatabase(xpEarned);
        }
    ...
    async updateXPInDatabase(xpEarned) {
        try {
            const apiBase = window.API_URL || 'http://52.88.234.65:8000';
            const userId = localStorage.getItem('userId');
            
            if (userId) {
                await fetch(`${apiBase}/api/users/${userId}/xp`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    updateStreak() {
        // Check if this is the first correct answer today
        ...
        if (lastStreakDate !== today) {
            ...
            if (streakElement) {
                ...
                localStorage.setItem('userStreak', currentStreak + 1);
    ...
    deductHeart() {
        // Get current hearts from dashboard
        const heartsElement = document.getElementById('livesValue');
        if (heartsElement) {
            const currentHearts = parseInt(heartsElement.textContent) || 5;
            const newHearts = Math.max(0, currentHearts - 1);
            
            // Animate the hearts update
            heartsElement.textContent = newHearts;
            heartsElement.style.color = '#ff4b4b';
            setTimeout(() => {
                heartsElement.style.color = '';
            }, 500);
        ...
        document.getElementById('answersContainer').innerHTML = `
            <div class="quiz-results">
                ...
                <div class="result-stat">
                    ...
                    <span class="stat-value">+${totalXP} XP</span>
                </div>
            </div>
